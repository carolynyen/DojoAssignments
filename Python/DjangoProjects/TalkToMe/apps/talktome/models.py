...
class RoomManager(models.Manager):
    ...
    def makeroom(self, postData, user):
        if len(postData['location']) < 1:
            ...
            check1 = Room.objects.annotate(count = (Count('users'))).filter(count = 1).filter(users__location=postData['location'], name=user.location)

            # check2 = Room.objects.annotate(count = (Count('users'))).filter(count = 1).filter(users__location=user.location, name = postData['location'])
